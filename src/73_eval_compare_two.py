...
rcParams['figure.figsize'] = 6.3, 4.2
...
def build_curves(mpeg_data, mpeg2_data, cmp_data, refined_cmp_data, refined_opt_cmp_data, ylabel, filename, cmp_path, ylim=None, xlim=None, legend_loc='lower right'):
    logger.info(f'Building curves with {ylabel}')
    logger.debug(f'mpeg_data: {mpeg_data}')
    logger.debug(f'mpeg2_data: {mpeg2_data}')
    logger.debug(f'cmp_data: {cmp_data}')

    mpeg_finite = np.isfinite(mpeg_data[:, 1])
    mpeg2_finite = np.isfinite(mpeg2_data[:, 1])

    if not np.all(mpeg_finite):
        mpeg_lossless = mpeg_data[~mpeg_finite]
        mpeg_lossless_bpp = np.min(mpeg_lossless[:, 0])
    if not np.all(mpeg2_finite):
        mpeg2_lossless = mpeg2_data[~mpeg2_finite]
        mpeg2_lossless_bpp = np.min(mpeg2_lossless[:, 0])

    mpeg_data = mpeg_data[mpeg_finite]
    mpeg2_data = mpeg2_data[mpeg2_finite]
    ...
    fig, ax = plt.subplots()
    ax.plot(mpeg_data[:, 0], mpeg_data[:, 1], label='GPCC v7', linestyle='-', marker='D')
    ax.plot(mpeg2_data[:, 0], mpeg2_data[:, 1], label='GPCC v3', linestyle='-', marker='o')
    if not np.all(mpeg_finite):
        ax.axvline(x=mpeg_lossless_bpp, label='GPCC v7 (lossless)', linestyle='-')
    if not np.all(mpeg2_finite):
        ax.axvline(x=mpeg2_lossless_bpp, label='GPCC v3 (lossless)', linestyle='-')
    ...
    bdrate2 = bd.bdrate(mpeg2_data, cmp_data)
    refined_bdrate2 = bd.bdrate(mpeg2_data, refined_cmp_data)
    refined_opt_bdrate2 = bd.bdrate(mpeg2_data, refined_opt_cmp_data)
    message = f'bdrate {ylabel}: {bdrate}\n' +\
              f'refined bdrate {ylabel}: {refined_bdrate}\n' +\
              f'refined opt bdrate {ylabel}: {refined_opt_bdrate}\n' +\
              f'bdrate2 {ylabel}: {bdrate2}\n' + \
              f'refined bdrate2 {ylabel}: {refined_bdrate2}\n' + \
              f'refined opt bdrate2 {ylabel}: {refined_opt_bdrate2}\n'
    logger.info(message)
    with open(os.path.join(cmp_path, 'eval.log'), 'a') as f:
        f.write(message + '\n')


def run(mpeg_path, mpeg2_path, cmp_path):
    assert os.path.exists(mpeg_path), f'{mpeg_path} does not exist'
    assert os.path.exists(mpeg2_path), f'{mpeg2_path} does not exist'
    ...
    mpeg2_reports = glob(os.path.join(mpeg2_path, '**/report.json'), recursive=True)
    ...
    logger.info(f'mpeg2_reports: {mpeg2_reports}')
    ...
    mpeg2_reports = [read_json(x) for x in mpeg2_reports]
    ...
    mpeg2_df = pd.DataFrame(data=mpeg2_reports)
    ...
    for (filename, column, ylabel, legend_loc) in curves:
        build_curves(
            ...
            mpeg2_df[['color_bits_per_input_point', column]].values,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog='73_eval_compare_two.py', description='Gathers reports and produces summary.',
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('mpeg_path', help='MPEG test folder.')
    parser.add_argument('mpeg2_path', help='MPEG test folder.')
    parser.add_argument('cmp_path', help='Comparison method test folder.')
    args = parser.parse_args()

    run(args.mpeg_path, args.mpeg2_path, args.cmp_path)
